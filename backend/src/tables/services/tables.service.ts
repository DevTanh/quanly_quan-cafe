...
export class TablesService {
  ...
  async findZoneById(id: number) {
    ...
    if (!zone) throw new NotFoundException(`Zone #${id} khong ton tai`)
  ...
  async updateZone(id: number, dto: UpdateZoneDto) {
    ...
    if (!existing) throw new NotFoundException(`Zone #${id} khong ton tai`)
  ...
  async findTableById(id: number) {
    ...
    if (!table) throw new NotFoundException(`Table #${id} khong ton tai`)
    return this.toTableResponse(table)
  }

  async createTable(dto: CreateTableDto) {
    const zone = await this.zonesRepo.findById(dto.zoneId)
    if (!zone) throw new NotFoundException(`Zone #${dto.zoneId} khong ton tai`)
    if (zone.status !== ZoneStatus.ACTIVE) {
      throw new BadRequestException("Khong the tao ban trong khu vuc da ngung hoat dong")
    }
  ...
  async updateTable(id: number, dto: UpdateTableDto) {
    ...
    if (!existing) throw new NotFoundException(`Table #${id} khong ton tai`)

    if (dto.zoneId != null) {
      const zone = await this.zonesRepo.findById(dto.zoneId)
      if (!zone) throw new NotFoundException(`Zone #${dto.zoneId} khong ton tai`)
      if (zone.status !== ZoneStatus.ACTIVE) {
        throw new BadRequestException("Khong the chuyen ban vao khu vuc da ngung hoat dong")
      }
    }
