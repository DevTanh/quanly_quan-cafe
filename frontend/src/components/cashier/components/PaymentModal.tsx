...
interface Props {
  ...
  onDiscountChange: (value: number, type: 'fixed' | 'percent', grossTotal: number) => void;
  onRedeemChange: (points: number) => void;  // NEW
}
...
const PaymentModal: React.FC<Props> = ({
  modal, paying, selectedCustomer,
  onClose, onConfirm, onCancelQr,
  ...
  const change = modal.method === 'cash'
    ? Math.max(0, modal.receivedAmount - modal.total) : 0;
  ...
  const canConfirm =
    !paying && !isQrPolling && !isQrPaid &&
    !(modal.method === 'cash' && modal.receivedAmount < modal.total);

  const handleDiscountApply = () => {
    const val = parseFloat(discountInput);
    if (isNaN(val) || val < 0) return;
    onDiscountChange(val, discountType, modal.grossTotal);
  };
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-2xl shadow-2xl w-[480px] max-h-[92vh] overflow-y-auto"
      ...
      >
        ...
        <div className="px-6 py-4 space-y-4">
            ...
            <div className="text-center py-8 space-y-3">
              ...
              {selectedCustomer && (
                <p className="text-[13px] text-[#6b6b68]">
                  Đã tích điểm cho <strong>{selectedCustomer.name}</strong>
                </p>
              )}
              <p className="text-[12px] text-[#a8a8a3]">Đang đóng tự động...</p>
            </div>
          )}
            ...
            <div className="space-y-4 text-center">
              {isQrFailed ? (
                <div className="py-6 space-y-3">
                  ...
                  <button onClick={onCancelQr} className="px-4 py-2 bg-[#f6f6f4] rounded-lg text-[13px] text-[#6b6b68] hover:bg-[#ebebea] transition-colors">
                ...
                <>
                  ...
                  {modal.checkoutUrl && (
                    <a href={modal.checkoutUrl} target="_blank" rel="noopener noreferrer"
                      className="inline-flex items-center gap-1 text-[12px] text-blue-500 hover:underline">
                      Mở trang PayOS ↗
                    </a>
                  )}
                  <button onClick={onCancelQr}
          ...
          {!showQrScreen && !isQrPaid && (
            <>
              {/* Total */}
              <div className="bg-[#f6f6f4] rounded-xl px-5 py-4">
                ...
                {/* Breakdown nếu có giảm giá */}
                {(modal.discount > 0 || modal.redeemDiscount > 0) && (
                  <div className="mt-2 pt-2 border-t border-[#ebebea] space-y-1">
                    <div className="flex justify-between text-[12px]">
                      <span className="text-[#a8a8a3]">Trước giảm</span>
                      <span className="font-mono text-[#a8a8a3]">{fmt(modal.grossTotal)}</span>
                    </div>
                    {modal.discount > 0 && (
                      <div className="flex justify-between text-[12px]">
                        <span className="text-green-600">Giảm giá</span>
                        <span className="font-mono text-green-600">−{fmt(modal.discount)}</span>
                      </div>
                    )}
                    {modal.redeemDiscount > 0 && (
                      <div className="flex justify-between text-[12px]">
                        <span className="text-purple-600">Dùng điểm ({modal.redeemedPoints.toLocaleString()} đ)</span>
                        <span className="font-mono text-purple-600">−{fmt(modal.redeemDiscount)}</span>
                      </div>
                    )}
                  </div>
                )}
              ...
              {selectedCustomer && (
                <PointsPanel
                  ...
                  orderTotal={modal.grossTotal}
                  redeemedPoints={modal.redeemedPoints}
              ...
              {/* Discount */}
              <div>
                <p className="text-[11.5px] font-semibold text-[#a8a8a3] uppercase tracking-wide mb-2">Giảm giá</p>
                    ...
                    {(['fixed', 'percent'] as const).map(t => (
                      <button key={t}
                        ...
                        className={[
                          ...
                          discountType === t ? 'bg-[#111110] text-white' : 'bg-white text-[#6b6b68] hover:bg-[#f6f6f4]',
                        ].join(' ')}
                      >
                        {t === 'fixed' ? '₫' : '%'}
                      </button>
                ...
                {modal.discount > 0 && (
                  <p className="text-[12px] text-green-600 mt-1.5">
                    ...
                    <button onClick={() => { setDiscountInput(''); onDiscountChange(0, 'fixed', modal.grossTotal); }}
                      className="ml-2 text-[11px] text-[#a8a8a3] underline">xóa</button>
                  </p>
                )}
              </div>
              ...
              {/* Method */}
              <div>
                <p className="text-[11.5px] font-semibold text-[#a8a8a3] uppercase tracking-wide mb-2">Phương thức</p>
                <div className="grid grid-cols-3 gap-2">
                  {METHOD_OPTS.map(opt => (
                    <button key={opt.value} onClick={() => onMethodChange(opt.value)}
                      className={[
                    ...
                    >
                      <span className={modal.method === opt.value ? 'opacity-90' : 'opacity-60'}>{opt.icon}</span>
                      {opt.label}
                    </button>
                  ))}
                </div>
              ...
              {modal.method === 'cash' && (
                <div className="space-y-3">
                  <div>
                    <label className="text-[11.5px] font-semibold text-[#a8a8a3] uppercase tracking-wide block mb-1.5">Khách đưa</label>
                    <div className="relative">
                      <input
                        ref={receivedRef} type="number" min={0} step={1000}
                    ...
                    <div className="flex gap-1.5 mt-2 flex-wrap">
                      {[modal.total, 50000, 100000, 200000, 500000].map(v => (
                        <button key={v} onClick={() => onReceivedChange(v)}
                  ...
                  <div className={['flex items-center justify-between rounded-xl px-4 py-3', change > 0 ? 'bg-green-50' : 'bg-[#f6f6f4]'].join(' ')}>
                    <span className={`text-[13px] font-medium ${change > 0 ? 'text-green-700' : 'text-[#6b6b68]'}`}>Tiền thừa</span>
                    <span className={`font-mono text-[18px] font-bold ${change > 0 ? 'text-green-700' : 'text-[#6b6b68]'}`}>{fmt(change)}</span>
                  </div>
                </div>
              )}
